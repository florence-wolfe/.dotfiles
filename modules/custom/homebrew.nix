...
let
    ...
    options = {
      cleanup = mkOption {
        ...
        description = ''
          ...
          the Brewfile generated by this module, during <command>nix</command> system
          activation.

          When set to <literal>"none"</literal> (the default), formulae not present in the generated
          Brewfile are left installed.

          When set to <literal>"uninstall"</literal>, <command>nix</command> invokes
          ...
          When set to <literal>"zap"</literal>, <command>nix</command> invokes
          ...
          If you plan on exclusively using <command>nix</command> to manage formulae
      ...
      autoUpdate = mkOption {
        ...
        description = ''
          ...
          <command>nix</command> system activation. The default is <literal>false</literal>
          so that repeated invocations of <command>home-manager switch</command> are idempotent.
    ...
    options = {
      ...
      autoUpdate = mkOption {
        ...
        description = ''
          ...
          your installed formulae will only be upgraded during <command>nix</command> system
  ...
  homebrewEnvironmentVariables = {
    HOMEBREW_BUNDLE_FILE = if config.brewfile then "${brewfileFile}" else "";
    HOMEBREW_NO_AUTO_UPDATE = if !config.autoUpdate then "1" else "";
    HOMEBREW_BUNDLE_NO_LOCK = if !config.lockfiles then "1" else "";
  };
};
...

{
  ...
  options.homebrew = {
    enable = mkEnableOption ''
      <command>nix</command> to manage installing/updating/upgrading Homebrew taps, formulae,
      ...
      Use the ${mkDocOptionLink "homebrew.brews"}, ${mkDocOptionLink "homebrew.casks"}, and ${mkDocOptionLink "homebrew.whalebrews"} options
      ...
      <command>nix</command> passes to the <command>brew bundle</command> command during
      system activation.
      ...
      invocations of <command>home-manager switch</command> (without any change to the
    ...
    onActivation = mkOption {
      ...
      description = ''
        ...
        <command>nix</command> runs during system activation.
      '';
    };
    ...
    brewfile = mkInternalOption {
      ...
      config = {
        ...
        warnings = [
          (mkIf (options.homebrew.autoUpdate.isDefined || options.homebrew.cleanup.isDefined) "The `homebrew' module no longer upgrades outdated formulae and apps by default during `nix' system activation. To enable upgrading, set `homebrew.onActivation.upgrade = true'.")
        ];

        homebrew.brews = optional (cfg.whalebrews != [ ]) "whalebrew";

        homebrew.brewfile =
          "# Created by `nix`'s `homebrew` module\n\n"
        ...
        home.activation.homebrew = lib.hm.dag.entryAfter ["writeBoundary"] (mkIf cfg.enable ''
          # Homebrew Bundle
